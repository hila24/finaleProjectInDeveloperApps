...
מי שיצרה את הסקר ממשיכה לראות אותו בהיסטוריה שלה.
...
1. **מהמסך — ברגע הדדליין, לכל מי שאינה היוצרת.** כל מסך שמצייר תמונה עובר דרך
   `Poll.showsImagesTo(uid)`. סקר סגור לא מציג תמונות בפיד ולא בארכיון של החברות —
   אבל **כן** ממשיך להציג אותן ליוצרת, בהיסטוריה שלה בפרופיל.
2. **מ‑Firestore — במועד עצמו, על המכשיר של הבעלים.** משימת ה‑WorkManager שמודיעה על
   סגירת הסקר מריצה גם את `cleanupExpiredPollsOf`: מסמכי **התמונות המלאות** נמחקים
   ו‑`imagesDeleted` מסומן. הממוזערות (`thumb`) **נשארות** על מסמך הסקר — הן ~12KB מול
   ~930KB, כך שכמעט כל המקום מתפנה והיוצרת עדיין רואה את ההיסטוריה שלה.
