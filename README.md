...
ההיעלמות היא שני שלבים נפרדים, ובכוונה:

1. **מהמסך — ברגע הדדליין.** כל מסך שמצייר תמונה עובר דרך `Poll.showsImages`, ולכן סקר
   סגור לא מציג תמונה בשום מקום: לא בפיד, לא בארכיון בפרופיל ולא במסך התוצאות. זה לא
   תלוי באף אחד ולא ברשת.
2. **מ‑Firestore — במועד עצמו, על המכשיר של הבעלים.** משימת ה‑WorkManager שמודיעה על
   סגירת הסקר מריצה גם את `cleanupExpiredPollsOf`: מסמכי התמונות נמחקים, `thumb`
   מתרוקן ו‑`imagesDeleted` מסומן — התוצאות נשארות.
3. **גיבוי — בפעם הבאה שהבעלים פותחת את האפליקציה.** אותה פעולה בדיוק נקראת גם מהפיד.

רק הבעלים מורשית למחוק את התמונות של הסקר שלה, ולכן שני השלבים האחרונים רצים רק על
המכשיר שלה. ה‑Worker מקבל את `ownerId` בעת התזמון, ומשווה אותו למשתמש המחובר בזמן
הריצה — הטלפון של חברה מתזמן משימה לאותו סקר, אבל לא ינסה למחוק.

השלב השלישי לא מיותר: WorkManager עלול להתעכב ב‑Doze, ו‑Force Stop מבטל משימות ממתינות
לגמרי. שתי הדרכים אידמפוטנטיות, אז לא משנה מי מהן מגיעה ראשונה.

## בדיקות

22 בדיקות יחידה על הלוגיקה שקשה לתפוס בעין — חישוב הסקר (מנצחת, אחוזים, ממוצע דירוג,
...
`PollTest.kt` מכיל 17 בדיקות ו‑`DeadlinesTest.kt` עוד 5. נבחרה דווקא הלוגיקה הזו כי
...
57 בדיקות נוספות בתיקייה `firestore-tests/` רצות מול **אמולטור Firestore** שטוען את
