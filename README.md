...
2. **מ‑Firestore — בפעם הבאה שהבעלים פותחת את האפליקציה.**
   `PollRepository.cleanupExpiredPollsOf` נקרא מהפיד: עבור כל סקר שלה שהדדליין שלו עבר
   נמחקים מסמכי התמונות, `thumb` מתרוקן ו‑`imagesDeleted` מסומן — התוצאות נשארות.

השלב השני מחכה לבעלים כי רק היא מורשית למחוק את התמונות של הסקר שלה. בלי השלב הראשון
היה נוצר חלון שבו חברה יכולה לפתוח סקר שנסגר מהארכיון ועדיין לראות את התמונות.
